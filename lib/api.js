...
export const deleteScreening = async (screeningId) => {
  ...
  if (!response.ok && response.status !== 204) {
    throw new Error("Impossible de supprimer le screening.");
  }
...
export const addToWatchlist = async (ticker, notes = null) => {
  const response = await fetch(`${API_BASE_URL}/watchlist`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(errorData.detail || "Impossible d'ajouter à la watchlist.");
  }
  return response.json();
};
...
export const removeFromWatchlist = async (itemId) => {
  ...
  if (!response.ok && response.status !== 204) {
    throw new Error("Impossible de supprimer de la watchlist.");
  }
  // No content is returned on successful deletion (204)
  return true;
};